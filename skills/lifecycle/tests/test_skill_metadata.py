...
class SkillMetadataTests(unittest.TestCase):
    def test_openai_yaml_interfaces_follow_skill_creator_contract(self) -> None:
        ...
        for path in paths:
            with self.subTest(skill=path.parents[1].name):
                ...
                self.assertTrue(set(value).issubset({"interface", "dependencies", "policy"}))
    ...
    def test_plugin_manifest_routes_to_all_skills_with_valid_starter_prompts(self) -> None:
    ...
    def test_bundled_python_tools_declare_requirements_when_they_import_third_party_packages(self) -> None:
        expected = {
            "google-mail": {"google-api-python-client", "google-auth", "google-auth-oauthlib"},
        ...
        paths = {
            ...
            "work-unit-planner": SKILLS / "work-unit-planner" / "assets" / "scripts" / "requirements.txt",
        }
