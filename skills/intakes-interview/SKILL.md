...
participant-session interpretation to `intakes-research`. Do not store research
participant evidence as kind `interview` or force a research session into the
three-choice decision format.

## Core Rules

- Apply the Interview Decision Gate from the bundled `factories-rule` skill's
  `references/interview-decision-gate.md` before using this skill. Resolve it
  under `../factories-rule/` relative to this skill according to
  `factories-lifecycle`.
...
relative to the shell working directory or the `intakes-interview` skill directory.
