...
Apply `factory-rule`. Do not treat a participant statement as an
observed behavior or generalize beyond the sampled people and situations.

## Boundary

- Use `intake-interview` for Human-only project and lifecycle decisions.
- Use `intake-analysis` for repository, database, analytics, log, test, runtime, and
  document evidence that does not require observing a person.
- Use `intake-web-search` for public or external published evidence.
...
relative to the shell working directory or the `intake-research` skill directory.
