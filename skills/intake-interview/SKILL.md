...
participant-session interpretation to `intake-research`. Do not store research
participant evidence as kind `interview` or force a research session into the
three-choice decision format.

## Core Rules

- Apply the Interview Decision Gate from the bundled `factory-rule` skill's
  `references/interview-decision-gate.md` before using this skill. Resolve it
  under `../factory-rule/` relative to this skill according to
  `factory-lifecycle`.
...
relative to the shell working directory or the `intake-interview` skill directory.
