...
def sessions_root(project_root: Path, create: bool = True) -> Path:
    root = project_root / ".agent-factory" / "sessions"
    if create:
...
def agent_directory(project_root: Path, agent_id: str, create: bool = False) -> Path:
    ...
    path = sessions_root(project_root, create=create) / agent_id
    if create:
...
def iter_agent_directories(root: Path) -> Iterator[Path]:
    sessions = sessions_root(root, create=False)
