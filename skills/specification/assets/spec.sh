...
def request_file(target: str) -> tuple[Path, bool]:
    ...
    candidate = specification_root.joinpath(*relative.parts).resolve(strict=False)
    try:
        candidate.relative_to(specification_root)
...
print(f"Serving {specification_root} read-only at {url}")
webbrowser.open(url)
try:
