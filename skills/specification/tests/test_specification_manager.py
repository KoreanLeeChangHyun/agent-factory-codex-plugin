...
def create_package(root: Path, profile_id: str = "project-core", specification_id: str = "sample-spec") -> Path:
...
class SpecificationManagerTests(unittest.TestCase):
    def test_check_schemas_validates_every_implemented_profile(self) -> None:
        ...
        for path in PROFILE_ROOT.glob("*.profile.json"):
            self.assertEqual(json.loads(path.read_text(encoding="utf-8"))["implementationStatus"], "implemented")
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for profile_id in PROFILE_IDS:
                with self.subTest(profile=profile_id):
                    ...
                    profile = json.loads((PROFILE_ROOT / f"{profile_id}.profile.json").read_text(encoding="utf-8"))
                    expected = COMMON_SECTIONS + [entry["id"] for entry in profile["profileRequiredSections"]]
                    metadata = json.loads((package / "data" / "metadata.json").read_text(encoding="utf-8"))
                    toc = json.loads((package / "data" / "table-of-contents.json").read_text(encoding="utf-8"))
                    self.assertEqual(metadata["artifactType"], "specification")
                    self.assertEqual(metadata["documentClass"], profile["documentClass"])
                    self.assertEqual(metadata["documentProfile"], {"id": profile_id, "version": "1.0.0"})
                    self.assertEqual(metadata["lifecycle"], {"phase": "specification", "status": "draft"})
                    self.assertNotIn("readiness", metadata)
                    self.assertEqual([entry["id"] for entry in toc["sections"]], expected)
                    result = json.loads(run_cli("validate", str(package), "--full").stdout)
                    self.assertEqual(result["profile"], f"{profile_id}@1.0.0")
                    self.assertEqual(result["profileValidation"], "base-valid-and-profile-valid")
    ...
    def test_mutation_without_readiness_increments_one_revision(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertNotIn("readiness", metadata)
            shown = json.loads(run_cli("show", str(package), "--section", "purpose-and-scope").stdout)
    ...
    def test_profile_version_and_document_class_mismatch_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for field, value in (("documentProfile", {"id": "project-core", "version": "9.0.0"}), ("documentClass", "requirements")):
                with self.subTest(field=field):
                    package = create_package(root, specification_id=f"mismatch-{field.lower()}")
    ...
    def test_profile_fields_cannot_be_mutated_directly(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for field in ("documentClass", "documentProfile"):
                result = run_cli("metadata-set", str(package), field, "{}", check=False)
                self.assertNotEqual(result.returncode, 0)
    ...
    def test_legacy_custom_manifest_is_not_relabelled_as_conforming(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            package = Path(temporary) / ".agent-factory" / "specifications" / "legacy-spec"
            (package / "data").mkdir(parents=True)
            (package / "data" / "manifest.json").write_text('{"status":"draft"}\n', encoding="utf-8")
            result = run_cli("validate", str(package), check=False)
            self.assertNotEqual(result.returncode, 0)
    ...
    def test_profile_selection_does_not_leak_between_processes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(json.loads(run_cli("validate", str(api)).stdout)["profile"], "api-design@1.0.0")
            self.assertEqual(json.loads(run_cli("validate", str(core)).stdout)["profile"], "project-core@1.0.0")
