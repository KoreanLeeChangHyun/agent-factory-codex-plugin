...
def resolve(source, destination=None, project_root=None):
    ...
    print(
        json.dumps({"event": "destination-resolved", **result}, ensure_ascii=False),
        flush=True,
    )
...
def external_private_path(path, project_root, label):
    """Normalize a credential path and keep it outside the Git project."""
    candidate = Path(path).expanduser()
    if not candidate.is_absolute():
        candidate = Path(os.path.abspath(candidate))
    project = Path(project_root).resolve()
    if _is_within(candidate, project) or _is_within(
        candidate.resolve(strict=False), project
    ):
        raise RuntimeError(f"{label} must be outside the Git project: {candidate}")
    return candidate


def _open_absolute_parent(path, *, create):
...
def read_private_text(path, project_root, label, *, required=False):
    target = external_private_path(path, project_root, label)
    try:
        parent = _open_absolute_parent(target, create=False)
    except FileNotFoundError:
        if required:
            raise RuntimeError(f"{label} not found: {target}") from None
        return None
    descriptor = -1
    try:
        try:
        ...
        if not stat.S_ISREG(os.fstat(descriptor).st_mode):
            raise RuntimeError(f"{label} must be a regular non-symlink file: {target}")
        with os.fdopen(descriptor, "r", encoding="utf-8") as stream:
            descriptor = -1
            return stream.read()
    except OSError as error:
        raise RuntimeError(
            f"cannot safely read {label} as a regular non-symlink file: {target}: {error}"
        ) from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def read_private_json(path, project_root, label, *, required=False):
    content = read_private_text(path, project_root, label, required=required)
    if content is None:
        return None
    try:
...
def write_private_text(path, payload, project_root, label):
    """Atomically publish a user-only secret without following symlinks."""
    target = external_private_path(path, project_root, label)
    parent = _open_absolute_parent(target, create=True)
    temporary_name = None
    try:
        try:
            existing = os.open(
        ...
        if existing >= 0:
            try:
                ...
                os.close(existing)
        for _ in range(32):
            temporary_name = f".credential.{secrets.token_hex(12)}"
            try:
                descriptor = os.open(
                    temporary_name,
                    os.O_WRONLY
            ...
            raise RuntimeError(f"cannot allocate temporary file for {label}")
        with os.fdopen(descriptor, "w", encoding="utf-8") as stream:
            stream.write(payload)
            stream.flush()
            os.fsync(stream.fileno())
        os.replace(temporary_name, target.name, src_dir_fd=parent, dst_dir_fd=parent)
        temporary_name = None
        os.fsync(parent)
    finally:
