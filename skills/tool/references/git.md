...
Use the Agent's existing shell/file tools, without a new local Tool MCP/provider service; the former local Tool adapter is retired. Resolve the executable and explicit project target, then inspect bounded versions and non-mutating repository facts. Preserve worktree root, Git directory, common directory, branch and detached state separately.

For one explicit hostname, use the provider-supported non-secret `gh auth status --hostname HOST --json hosts` interface when supported and retain only bounded account/scope facts, never token output. Executable availability, structured-inspection support and authentication are separate observations. Unsupported fields or unparseable responses leave authentication unknown; only a supported response establishes available or unavailable authentication. Git LFS activation and remote access remain unknown unless separately observed.

Actual package/auth/repository/remote mutations stay with their owning authority and require the authorized target/effects. Discovery is not permission to perform them. Upload only required non-secret inspection evidence through the authenticated Document tools under a bound cloud target.
