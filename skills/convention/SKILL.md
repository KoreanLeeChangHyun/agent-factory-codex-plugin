...
Exactly six public distributed Skills remain `agent`, `convention`, `document`, `gather`, `tool`, `workspace`. Explorer and Interview are Convention-owned capabilities applied by Agent roles, not extra public Skills or roles. Main orchestrates, Work performs bounded work without self-verification or commit, and Verification independently checks without repair. Only independent pass or evidenced Human skip applied after Work completion reaches END. Main alone integrates an authorized ordinary Git commit for the exact verified/skipped paths; this never grants push, amend, history rewrite or unrelated mutation.
...
Every Specification has one AI Skill and one Korean Human HTML/CSS/JavaScript body under one stable identity. Preserve complete meaning, order, hierarchy and provenance with exact source and contiguous line hashes. Partial, one-sided, stale, duplicated, reordered, summarized or mistranslated pairs cannot be completed. Independent semantic review is separate from hash coverage. Document owns the full paired publication and migration contracts.
...
Workspace is the Human control tower with exactly six ordered Activities: 일정, 에이전트, 문서, 외부연동, 로그, 테스트. Its detailed accepted architecture, planning and unresolved Human-owned decisions belong to the Workspace references. The MCP application owns its shell/assets; do not copy a browser runtime into consumer projects. Tool owns logical lifecycle routing while actual hosts/providers and cloud credential authority retain their state. Tool readiness grants no Agent execution authority and no automatic scope escalation.

## Reference routing

- `references/agent-factory-core.md`: core roles, authority, Documents, pairing, engineering layers and accepted decisions.
