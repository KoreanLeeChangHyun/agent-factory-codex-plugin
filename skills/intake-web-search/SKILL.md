...
runtime investigation to `intake-analysis`. Route direct observation, contextual
inquiry, workflow shadowing, usability sessions, and consented participant
session review to `intake-research`. A published user research report found on the
...
- Do not create a separate Markdown, HTML, or JSON intake-web-search source of truth.
...
relative to the shell working directory or the `intake-web-search` skill directory.
