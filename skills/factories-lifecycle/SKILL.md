...
Use `intakes-research` when Intake evidence requires direct observation of users
or operators.
...
The primary `agents-main` makes one sufficiency decision immediately before
launch:
...
`work-units-execution/scripts/app_server_goal.py` as a background process. The
...
- `factories-rule` before edits, design, code, review, or workflow claims.
- `intakes` for canonical Intake.
- `intakes-analysis`, `intakes-web-search`, and `intakes-research` for evidence collected into
  Intake.
- `specifications` for canonical Specification.
- `work-units-manager` for Work Unit creation and state.
- `work-units-execution` for Goal launcher and Git worktree operations.
- `agents-workflow` for launched execution.
