...
SCHEMA_PATH = (
    SKILLS / "factory-lifecycle" / "assets" / "schema" / "document-profile.schema.json"
)
PROFILE_PATHS = sorted(
    (SKILLS / "specifications" / "assets" / "profiles").glob("*.profile.json")
) + [SKILLS / "work-unit-manager" / "assets" / "profiles" / "work-unit.profile.json"]
...
class DocumentProfileTests(unittest.TestCase):
    ...
    def test_work_unit_v4_is_implemented_and_anchored(self) -> None:
        ...
        planner = (SKILLS / "work-unit-manager" / "SKILL.md").read_text(
            encoding="utf-8"
        )
        common = (
            SKILLS / "factory-lifecycle" / "references" / "common-document-contract.md"
    ...
    def test_sectioned_document_manager_is_owned_by_lifecycle(self) -> None:
        common_manager = (
            SKILLS / "factory-lifecycle" / "assets" / "scripts" / "sectioned_document.py"
        )
        common_schemas = (
            SKILLS / "factory-lifecycle" / "assets" / "schema" / "sectioned-document"
        )
        ...
        work_unit_manager = (
            SKILLS / "work-unit-manager" / "assets" / "scripts" / "work_unit.py"
        )
    ...
    def test_project_core_policy_is_single_source_and_profile_driven(self) -> None:
        ...
        lifecycle = (SKILLS / "factory-lifecycle" / "references" / "lifecycle.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_traceability_starts_at_ready_intake_and_specification_is_conditional(
        ...
        factory_rule = (SKILLS / "factory-rule" / "SKILL.md").read_text(encoding="utf-8")
        self.assertNotIn(
    ...
    def test_specification_supporting_sources_use_registered_blocks(self) -> None:
        ...
        diagram = (SKILLS / "specification-diagram" / "SKILL.md").read_text(encoding="utf-8")
