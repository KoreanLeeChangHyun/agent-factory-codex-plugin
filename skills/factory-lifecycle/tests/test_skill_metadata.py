...
class SkillMetadataTests(unittest.TestCase):
    ...
    def test_lifecycle_starts_with_intake_without_init_skill_or_mandatory_specification(
        ...
        lifecycle = (SKILLS / "factory-lifecycle" / "SKILL.md").read_text(encoding="utf-8")
        lifecycle_reference = (
            SKILLS / "factory-lifecycle" / "references" / "lifecycle.md"
    ...
    def test_annotation_convention_has_single_skill_owner(self) -> None:
        ...
        self.assertEqual(
            owners,
            [SKILLS / "convention-annotation" / "SKILL.md"],
        )
        factory_rule = (SKILLS / "factory-rule" / "SKILL.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("Use `convention-annotation`", factory_rule)
    ...
    def test_bundled_python_tools_declare_requirements_when_they_import_third_party_packages(
        ...
        expected = {
            ...
            "work-unit-manager": {"jsonschema>=4.18,<5"},
        }
        paths = {
            "sync-google-gmail": SKILLS / "sync-google-gmail" / "scripts" / "requirements.txt",
            "intakes": SKILLS / "intakes" / "scripts" / "requirements.txt",
            "specifications": SKILLS / "specifications" / "scripts" / "requirements.txt",
            "work-unit-manager": SKILLS
            / "work-unit-manager"
            / "assets"
            / "scripts"
            / "requirements.txt",
